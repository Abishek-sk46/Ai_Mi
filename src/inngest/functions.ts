...
import { Agent, createAgent, createNetwork, createTool, gemini } from "@inngest/agent-kit";
        ...
        handler: async ({ command }, { step }) => {
          return await step?.run("terminal", async () => {
            ...
            try {
              ...
              console.error(
                `Command failed: ${command} with error: ${e} \nstdout: ${buffers.stdout} \nstderr: ${buffers.stderr}`
              )
            }
          });
        }
          ...
          const newFiles = await step?.run("CreateOrUpdateFiles", async () => {
            try{
              ...
              const sandbox = await getSandbox(sandboxId);
                ...
                function sanitizeCode(content: string) {
                  return content.replace(/```(typescript|tsx|js|javascript)?/g, "").trim();
              }
